...
What if I told you the cost of manipulating a TWAP is overlooked, and there’s a way to reduce it? (in fact UniV3 did it) That’s what is explained in this paper: https://eprint.iacr.org/2022/445.pdf 
...
What make attack on a TWAP difficult is that the funds must be available over multiple blocks. This means attacker must be rich, while also risking to see his positions arbitrated. The longer is the TWAP window, the riskier it become.
...
$m_p$ for just one block to $(1 + L_T · ϵ) · mp$
