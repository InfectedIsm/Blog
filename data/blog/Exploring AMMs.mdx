...
1. **Borrow** from a CDP protocol $x_A$ of $token_A$ such as $x_A.p_A = x_B.p_B$ (where $p$ is the token price in numeraire, or dollars for simplicity) at market price.
...
3. **Borrow**, from another CDP protocol using the AMM as sole price oracle, $x_A + \Delta_2$ of $token_A$ with  $\alpha .x_B$  of $token_B$ obtained in step (2)
	- Because $p_{A/B}$ has been pushed down, now $token_A$ is worth more less, making it possible to borrow more $x_A$ than in step (1)
4. **Repay** the flashloan with $x_A$ of $token_A$
...
4. **Withdraw liquidity** by surrending LP tokens and receive:
